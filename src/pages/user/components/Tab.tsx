...
const classNameMappings = {
  active: 'basis-1/3 rounded-lg px-6pxr py-8pxr shadow-default bg-white hover:bg-white hover:text-gray-700',
  inactive: 'basis-1/3 rounded-lg px-6pxr py-8pxr text-gray-500 hover:bg-white hover:text-gray-700 hover:shadow',
};
...
const Tab = () => {
  ...
  return (
    <>
      <div className="mx-14pxr mt-37pxr">
        <div className="p-6pxr w-full rounded-[12px] bg-gray_150 flex flex-row justify-center items-center">
          <ul className="flex flex-row gap-5pxr text-center text-sm text-foot w-full">
            {tabs.map((tab) => (
              <TabItem key={tab.path} label={tab.label} path={tab.path} currentPath={currentPath} />
            ))}
          </ul>
        </div>
      </div>
